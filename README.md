...
2. Download and Install MySQL (You can download the community version from the Oracle website)
3. The Unit Test folder contains Unit Tests that are for Microsoft C++ Unit Testing Framework and will only work with Visual Studio
4. In the Code you may have noticed that where there is a password there is "getenv", and the server details. You will need to use a server of your own and use your own password and details there instead.
	1. Define ipapiAPIKey in your environment ( Sign up at https://ipapi.com/ )
	2. Define OpenWeatherAPIKey in your environment ( Sign up at https://home.openweathermap.org/users/sign_up and then navigate to API keys )
5. To setup the SQL Database for this go to the SQL Database Section below to see how the tables are setup.
